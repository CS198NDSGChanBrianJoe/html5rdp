...
int GetMilliSpanq(int nTimeStart){
    int nSpan = GetMilliCountq() - nTimeStart;
    if(nSpan < 0){
        nSpan += 0x100000 * 1000;
        return nSpan;
    }
...
int __guac_handle_sync(guac_client* client, guac_instruction* instruction) {
    ...
    /*ADDED CODE
    int nTimeStart=GetMilliCountq();
    ADDED CODE*/
    /*ADDED CODE
    int mtime = GetMilliCountq(); 
    int nTimeElapsed=GetMilliSpanq(nTimeStart);
    char str[40];
