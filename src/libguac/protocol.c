...
int __guac_socket_write_length_png(guac_socket* socket, cairo_surface_t* surface) {
    ...
    openlog("slog", LOG_PID|LOG_CONS, LOG_USER);
    syslog(LOG_INFO,"Just Entered @...write_length_png");
    closelog();
...
int guac_protocol_send_png(guac_socket* socket, guac_composite_mode mode,
    ...
    openlog("slog", LOG_PID|LOG_CONS, LOG_USER);
    syslog(LOG_INFO,"Just Entered @...protocol_send_png");
    closelog();
...
int GetMilliSpanz(int nTimeStart){
    int nSpan = GetMilliCountz() - nTimeStart;
    if(nSpan < 0){
        nSpan += 0x100000 * 1000;
        return nSpan;
    }
...
int guac_protocol_send_sync(guac_socket* socket, guac_timestamp timestamp) {
    ...
    /*ADDED CODE
    int nTimeStart=GetMilliCountz();
    ADDED CODE*/
    /*ADDED CODE
    int mtime = GetMilliCountz(); 
    int nTimeElapsed=GetMilliSpanz(nTimeStart);
    char str[40];
    sprintf(str,"%d",nTimeElapsed); 
    openlog("slog", LOG_PID|LOG_CONS, LOG_USER);
    syslog(LOG_INFO,"mtime-%d  gprocbs-%d",mtime,nTimeElapsed);
    closelog();
    ADDED CODE*/
