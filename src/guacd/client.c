...
int GetMilliSpanx(int nTimeStart){
    int nSpan = GetMilliCountx() - nTimeStart;
    if(nSpan < 0){
        nSpan += 0x100000 * 1000;
        return nSpan;
    }
...
void* __guacd_client_output_thread(void* data) {
    ...
    while (client->state == GUAC_CLIENT_RUNNING) {
        ...
        if (timestamp - last_ping_timestamp > GUACD_SYNC_FREQUENCY) {
            ...
            /*ADDED CODE
            int nTimeStart=GetMilliCountx();
            ADDED CODE*/
            /*ADDED CODE
            int mtime = GetMilliCountx(); 
            int nTimeElapsed=GetMilliSpanx(nTimeStart);
            char str[40];
            sprintf(str,"%d",nTimeElapsed); 
            openlog("slog", LOG_PID|LOG_CONS, LOG_USER);
            syslog(LOG_INFO,"mtime-%d  guuuprocbs-%d",mtime,nTimeElapsed);
            closelog();
            ADDED CODE*/
        ...
        if (client->handle_messages) {
            ...
            sprintf(str,"%d",nTimeElapsed); 
            openlog("slog", LOG_PID|LOG_CONS, LOG_USER);
            syslog(LOG_INFO,"mtime-%d  gfffprocbs-%d",mtime,nTimeElapsed);
            closelog();
            ADDED CODE*/
            ...
            if (client->last_sent_timestamp - client->last_received_timestamp
                ...
                sprintf(str,"%d",nTimeElapsed); 
                openlog("slog", LOG_PID|LOG_CONS, LOG_USER);
                syslog(LOG_INFO,"mtime-%d  gjjjprocbs-%d",mtime,nTimeElapsed);
                closelog();
                ADDED CODE*/
