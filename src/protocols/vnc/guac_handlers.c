...
int GetMilliSpans(int nTimeStart){
    int nSpan = GetMilliCounts() - nTimeStart;
    if(nSpan < 0){
        nSpan += 0x100000 * 1000;
        return nSpan;
    }
...
int vnc_guac_client_key_handler(guac_client* client, int keysym, int pressed) {
    ...
    /*ADDED CODE*/
    //int mtime = GetMilliCounts(); 
    //int nTimeElapsed=GetMilliSpans(nTimeStart);
    //char str[40];
    //sprintf(str,"%d",nTimeElapsed); 
    //openlog("slog", LOG_PID|LOG_CONS, LOG_USER);
    //syslog(LOG_INFO,"mtime-%d  gprocf-%d",mtime,nTimeElapsed);
    //closelog();
    /*ADDED CODE*/
    return 0;
}
